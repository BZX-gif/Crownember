...
import { findUserByUsername, getBlockState } from "@/lib/social";
...
export async function GET(req: Request) {
  ...
  if (block.any) return NextResponse.json({ sealed: true, messages: [] });
  const rows = await db.select().from(directMessages).where(and(
    or(
      and(eq(directMessages.senderId, user.id), eq(directMessages.recipientId, other.id)),
      and(eq(directMessages.senderId, other.id), eq(directMessages.recipientId, user.id)),
    ),
    gt(directMessages.id, after),
  )).orderBy(directMessages.id).limit(80);
  const live = rows.filter((m) => m.createdAt.getTime() > messageCutoff().getTime());
  return NextResponse.json({ sealed: false, messages: live.map((m) => ({
    id: m.id,
    content: m.content,
...
export async function POST(req: Request) {
  ...
  if (!other) return NextResponse.json({ error: "Player not found." }, { status: 404 });
  if (other.id === user.id) return NextResponse.json({ error: "Notes to self go in the forum. 😄" }, { status: 400 });
  if (!content || content.length > MAX_DM_LENGTH) return NextResponse.json({ error: `Message must be 1-${MAX_DM_LENGTH} characters.` }, { status: 400 });
  if (containsInjection(content)) return NextResponse.json({ error: "Messages can't contain code or script tags. 🛡️" }, { status: 400 });
  const block = await getBlockState(user.id, other.id);
  if (block.any) return NextResponse.json({ error: "This channel is sealed. 🔒" }, { status: 403 });
  ...
  const [created] = await db.insert(directMessages).values({ senderId: user.id, recipientId: other.id, content }).returning();
  return NextResponse.json({ message: { id: created.id, content: created.content, createdAt: created.createdAt, mine: true, editable: true } });
}
...
export async function PUT(req: Request) {
  ...
  const [updated] = await db.update(directMessages).set({ content }).where(and(
    eq(directMessages.id, id),
    eq(directMessages.senderId, user.id),
    gt(directMessages.createdAt, cutoff),
  )).returning();
  if (!updated) return NextResponse.json({ error: "You can only edit your own messages within 3 minutes." }, { status: 403 });
  return NextResponse.json({ message: { id: updated.id, content: updated.content, createdAt: updated.createdAt, mine: true, editable: true } });
}
