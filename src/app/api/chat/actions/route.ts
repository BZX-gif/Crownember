...
async function findMessage(id: number) {
  const row = await db.select({ message: messages, author: users, room: rooms }).from(messages).innerJoin(users, eq(messages.userId, users.id)).innerJoin(rooms, eq(messages.roomId, rooms.id)).where(eq(messages.id, id)).limit(1);
  return row[0] ?? null;
}
async function readReactions(): Promise<Record<string, string>> {
  ...
  if (!row[0]) return {};
  try { const parsed = JSON.parse(row[0].value); return parsed && typeof parsed === "object" ? parsed : {}; } catch { return {}; }
}
async function writeReactions(value: Record<string, string>) {
  const encoded = JSON.stringify(value);
  await db.insert(settings).values({ key: REACTION_KEY, value: encoded }).onConflictDoUpdate({ target: settings.key, set: { value: encoded } });
}
export async function POST(req: Request) {
  ...
  if (type === "react") {
    ...
    if (previous === emoji) delete reactions[key]; else reactions[key] = emoji;
    await writeReactions(reactions);
    const counts = Object.fromEntries(ALLOWED_REACTIONS.map((item) => [item, Object.entries(reactions).filter(([k, value]) => k.startsWith(`${found.message.id}:`) && value === item).length]));
