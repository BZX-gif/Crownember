...
export async function POST(req: Request) {
  ...
  if (type === "react") {
    ...
    const count = Object.entries(reactions).filter(
      ([key, value]) => key.startsWith(`${found.message.id}:`) && value === 1,
    ).length;
  ...
  if (type === "edit") {
    ...
    if (Date.now() - found.message.createdAt.getTime() > EDIT_WINDOW_MS) {
      return NextResponse.json(
        { error: "Edit window expired. Messages can only be edited for 3 minutes." },
        { status: 403 },
      );
    }
    ...
    const [updated] = await db
      .update(messages)
      .set({ content })
      .where(and(eq(messages.id, found.message.id), eq(messages.userId, user.id)))
      .returning();
    return NextResponse.json({ ok: true, message: serializeMessage(updated, found.author) });
  }

  if (type === "reply") {
    if (!content || content.length > MAX_MESSAGE_LENGTH - found.author.username.length - 4) {
    ...
    const [created] = await db
      ...
      .values({
        ...
        userId: user.id,
        content: `↩ @${found.author.username}: ${content}`,
      })
      .returning();
    const [updated] = await db
      .update(users)
