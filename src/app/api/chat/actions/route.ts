...
async function findMessage(roomSlug: string, username: string, content: string) {
  ...
  const row = await db
    ...
    .where(
      and(
        eq(messages.roomId, room[0].id),
        eq(messages.userId, author[0].id),
...
async function writeReactions(value: Record<string, number>) {
  ...
  await db
    .insert(settings)
    .values({ key: REACTION_KEY, value: encoded })
    .onConflictDoUpdate({
      target: settings.key,
      set: { value: encoded },
    });
...
export async function POST(req: Request) {
  ...
  if (!roomSlug || !username || !message) {
    return NextResponse.json({ error: "Missing message information." }, { status: 400 });
  }
  ...
  if (type === "react") {
    ...
    const count = Object.entries(reactions).filter(
      ([k, value]) => k.startsWith(`${found.message.id}:`) && value === 1,
    ).length;
    return NextResponse.json({ ok: true, count, message: "❤️ Reaction saved." });
  }

  if (type === "edit") {
    if (found.message.userId !== user.id) {
    ...
    if (containsInjection(content)) {
      return NextResponse.json({ error: "Messages can't contain code or script tags. 🛡️" }, { status: 400 });
    }
    const [updated] = await db
      .update(messages)
      .set({ content })
      .where(eq(messages.id, found.message.id))
      .returning();
    return NextResponse.json({ ok: true, message: serializeMessage(updated, found.author) });
  }

  if (type === "reply") {
    if (!content || content.length > MAX_MESSAGE_LENGTH - username.length - 4) {
      return NextResponse.json({ error: "Reply is too long." }, { status: 400 });
    }
    if (containsInjection(content)) {
      return NextResponse.json({ error: "Replies can't contain code or script tags. 🛡️" }, { status: 400 });
    }
    const replyContent = `↩ @${username}: ${content}`;
    const [created] = await db
      .insert(messages)
      .values({ roomId: found.room.id, userId: user.id, content: replyContent })
      .returning();
    const [updated] = await db
      .update(users)
      .set({ xp: user.xp + XP_AWARDS.MESSAGE, lastSeenAt: new Date() })
