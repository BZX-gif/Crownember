...
async function findLiveReactionMessage(roomSlug: string, username: string, content: string) {
  const row = await db
    .select({ message: messages, author: users, room: rooms })
    .from(messages)
    .innerJoin(users, eq(messages.userId, users.id))
...
export async function POST(req: Request) {
  ...
  let found = Number.isInteger(rawId) && rawId > 0 ? await findMessage(rawId) : null;

  // Reactions can arrive from a stale client DOM. Resolve the live message
  // by room + author + exact content so a harmless UI race never produces
  // the misleading "Message no longer exists" error.
  if (type === "react" && !found) {
    found = await findLiveReactionMessage(
      String(body.room ?? "global"),
      String(body.username ?? ""),
      String(body.messageContent ?? "").trim(),
    );
  }
  ...
  if (type === "react") {
    ...
    const counts = Object.fromEntries(
      ALLOWED_REACTIONS.map((item) => [
        item,
        Object.entries(reactions).filter(
          ([k, value]) => k.startsWith(`${found.message.id}:`) && value === item,
        ).length,
      ]),
    );
