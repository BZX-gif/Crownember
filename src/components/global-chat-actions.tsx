...
export function GlobalChatActions() {
  ...
  async function refresh() {
    try {
      ...
      if (messagesRes.ok) { const data = await messagesRes.json(); if (Array.isArray(data.messages)) setMessages(data.messages as Msg[]); }
      if (reactionsRes.ok) { const data = await reactionsRes.json(); if (data.messages && typeof data.messages === "object") setReactions(data.messages); }
    } catch {}
  }
  useEffect(() => { void refresh(); const timer = window.setInterval(refresh, 2500); return () => window.clearInterval(timer); }, [room]);
  function findMessage(el: HTMLElement): Msg | null {
    const value = el.textContent?.trim() ?? "";
    return messages.filter((m) => value.includes(m.user.username) && value.includes(m.content)).at(-1) ?? null;
  }
  useEffect(() => {
    ...
    const onDblClick = (event: MouseEvent) => {
      ...
      if (!msg) return;
      event.preventDefault(); setOpenId(msg.id); setPickerId(null); setEditId(null); setReplyId(null); setText(""); setError("");
    };
  ...
  useEffect(() => {
    ...
    for (const row of rows) {
      ...
      const msg = findMessage(row); if (!msg) continue;
      const state = reactions[String(msg.id)]; if (!state) continue;
      const active = Object.entries(state.counts).filter(([, count]) => count > 0); if (!active.length) continue;
      const bar = document.createElement("div"); bar.dataset.crownReactions = "1"; bar.className = "mt-1.5 flex flex-wrap gap-1";
      for (const [emoji, count] of active) { const button = document.createElement("button"); button.type = "button"; button.className = `rounded-full border px-2 py-0.5 text-xs transition ${state.selected === emoji ? "border-orange-400/70 bg-orange-500/20" : "border-white/10 bg-white/5 hover:bg-white/10"}`; button.textContent = `${emoji} ${count}`; button.onclick = () => void react(msg.id, emoji); bar.appendChild(button); }
      row.appendChild(bar);
    }
  }, [messages, reactions]);
  async function react(id: number, emoji: string) {
    ...
    try {
      ...
      const data = await res.json(); if (!res.ok) { setError(data.error ?? "Reaction failed."); return; }
  ...
  async function submitAction(type: "edit" | "reply") {
    ...
    try {
      ...
      const data = await res.json(); if (!res.ok) { setError(data.error ?? `${type === "edit" ? "Edit" : "Reply"} failed.`); return; }
