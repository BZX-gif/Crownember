...
export function GlobalChatActions() {
  ...
  async function react(emoji: string) {
    setBusy(true);
    try {
      const res = await fetch("/api/chat/actions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "react", id: action.id, emoji }) });
  ...
  async function reply() {
    const text = window.prompt(`Reply to @${action.username}`);
    if (!text?.trim()) return;
    setBusy(true);
    try {
      const res = await fetch("/api/chat/actions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "reply", id: action.id, content: text.trim() }) });
      const data = await res.json();
      if (!res.ok) window.alert(data.error ?? "Reply failed."); else window.location.reload();
    } finally { setBusy(false); setAction(null); }
  }

  async function edit() {
    const text = window.prompt("Edit your message", action.content);
    if (text === null || !text.trim() || text.trim() === action.content) return;
    setBusy(true);
    try {
      const res = await fetch("/api/chat/actions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "edit", id: action.id, content: text.trim() }) });
      const data = await res.json();
      if (!res.ok) window.alert(data.error ?? "Edit failed."); else window.location.reload();
    } finally { setBusy(false); setAction(null); }
  }
    ...
    <div data-global-chat-actions className="fixed z-[9999] w-[232px] rounded-xl border border-white/15 bg-slate-950/95 p-1.5 shadow-2xl backdrop-blur" style={{ top: action.top, left: action.left }} onClick={(e) => e.stopPropagation()}>
      ...
      {picker && (
        <div className="grid grid-cols-8 gap-1 border-y border-white/10 px-2 py-2">
          {REACTIONS.map((emoji) => (
            <button key={emoji} type="button" disabled={busy} onClick={() => void react(emoji)} className="rounded-lg p-1.5 text-xl transition hover:scale-125 hover:bg-white/10 disabled:opacity-50" title={`React ${emoji}`}>{emoji}</button>
          ))}
        </div>
      )}
