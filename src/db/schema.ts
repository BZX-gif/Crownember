...
export const users = pgTable("users", {
  ...
  // Avatar bytes/mime stay outside normal users queries and are served through /api/profile/avatar.
  ...
  /** Developer perk — owner flips this per player in the database.
   *  Grants the signed terminal messages + the 👻 ghost avatar sign. */
...
export const rooms = pgTable("rooms", {
  ...
  color: text("color").notNull().default("#ff6a00"),
  /** Marks the password-sealed room. Name/slug-independent on purpose —
   *  the vault can be renamed ("OMEGA VAULT" etc.) without breaking the
   *  gate, voice notes or lock indicators. */
...
export const messages = pgTable("messages", {
  ...
  index("messages_room_id_idx").on(t.roomId, t.id),
...
export const topics = pgTable("topics", {
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
}, (t) => [index("topics_activity_idx").on(t.lastActivityAt), index("topics_category_idx").on(t.category)]);
...
export const friendships = pgTable("friendships", {
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
}, (t) => [index("friendships_pair_idx").on(t.requesterId, t.addresseeId), index("friendships_addressee_idx").on(t.addresseeId, t.status)]);
...
export const blocks = pgTable("blocks", {
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
}, (t) => [index("blocks_pair_idx").on(t.blockerId, t.blockedId), index("blocks_blocked_idx").on(t.blockedId)]);
...
export const directMessages = pgTable("direct_messages", {
  ...
  content: text("content").notNull(),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
}, (t) => [index("dm_out_idx").on(t.senderId, t.recipientId, t.id), index("dm_in_idx").on(t.recipientId, t.senderId, t.id), index("dm_created_idx").on(t.createdAt)]);
